...
曲线定义在唯一一处：`eq_common.py` 里的 `harman_nearfield_target`，`eq_make.py`
使用它，替换只需改这里。`eq_make.py` 里的低频提升上限同样反映的是一只
...
管线只认一种导出格式——REW 默认设置：原始分辨率（「使用测量结果原始分辨率」）、
无平滑。倍频程（ppo）导出一律拒收：逐倍频程重采样是管线自己的工作，只做一次、
结果确定，不交给导出对话框代劳。
...
这里的目标是 Audiofrog 官方车内曲线（随仓库附带 audiofrog_target_curve.csv，缺失时退回内置近似），输出写在固定的 127 个整数频点上，以兼容 wavelet 型车机。目标曲线叠加了一层个人偏好修剪：低频 −5 dB 搁架（75 Hz 拐点），`--bass-trim` / `--bass-trim-fc` 可调、`--bass-trim 0` 关闭。
车型目录建议加入 `.gitignore`。
